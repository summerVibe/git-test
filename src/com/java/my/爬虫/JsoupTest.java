...
//	private static final Logger LOGGER =Logger.getLogger(JsoupTest.class);
	
	public static void main(String[] args) throws IOException {
//		connectAc("http://www.acfun.cn/");
		getTxts("1");
	}
	/**
	 * ac 爬取主页
	...
	private static Document connectUrl(String url){
		...
		try {
			 document = Jsoup.connect(url)
				...
				.cookie("auth", "token")
				.timeout(30000000)
				.referrer("http://www.wangshuge.com/")
				.get();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private static void getTxts(String type) throws IOException{
		for(int i=1;i<=626;i++){
			System.out.println("http://www.wangshuge.com/books/list/"+type+"-"+i+".html-------------------------------");
			
			xiaoShuo("http://www.wangshuge.com/books/list/"+type+"-"+i+".html");
		}
	}
	/**
	 * 笔趣主页
	...
	private static void xiaoShuo(String url) throws IOException{
		Document document = connectUrl(url);
		Elements select = document.select("#content tr");
		
		
		for(int i = 0; i<select.size();i++){
			Element element = select.get(i);
			if(element.getElementsByTag("td").size()!=0){
				System.out.println("========"+element.getElementsByTag("td").get(0).child(0).html());
				System.out.println("++++++++"+element.getElementsByTag("td").get(0).child(0).attr("href"));
				xiaoShuoz(element.getElementsByTag("td").get(0).child(0).attr("href"));
			}
		}
